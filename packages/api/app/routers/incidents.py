...
class IncidentReport(TimestampMixin, Base):
    ...
    incident_status: Mapped[str] = mapped_column(
        String(50), nullable=False, default="ongoing"
    )
...
async def list_incidents(
    ...
    count_q = select(func.count(IncidentReport.id)).where(
...
@router.post(
    "/{incident_id}/export", dependencies=[Depends(require_role("admin", "auditor"))]
)
async def export_incident(
    ...
    if not incident or incident.organization_id != membership.organization_id:
        raise HTTPException(status_code=404, detail="Incident report not found")
    return {
        "format": "CSIRT_NIS2_Art23",
        "version": "2.2",
        "payload": incident.report_data or _build_payload_from_model(incident),
    }
...
def _build_payload(p: IncidentCreate) -> dict:
    return {
        ...
        "incident": {
            ...
            "severity": p.severity,
            "status": p.incident_status,
            "detected_at": (p.detected_at or datetime.now(timezone.utc)).isoformat(),
            "description": p.description,
        },
        "impact": {"affected_users": p.affected_users, "cross_border": p.cross_border},
        "timeline": {
            "early_warning_deadline": "24h from detection (Art. 23.4.a)",
